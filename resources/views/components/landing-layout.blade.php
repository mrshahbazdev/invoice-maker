...
<body class="bg-page text-txmain antialiased selection:bg-brand-100 selection:text-brand-700">
 ...
 <span
 class="ml-3 text-2xl font-bold text-brand-600">
 ...
 class="inline-flex items-center justify-center px-6 py-2.5 text-sm font-bold text-white bg-brand-600 rounded-full hover: hover: shadow-lg shadow-brand-200 transition-all transform hover:-translate-y-0.5">
 {{ __('Go to Dashboard') }}
 </a>
 @else
 <a href="{{ route('login') }}"
 ...
 @else
 <span
 class="text-xl font-bold text-brand-600">
 {{ \App\Models\Setting::get('site.name', config('app.name', 'InvoiceMaker')) }}
 </span>
 @endif
 </a>
 ...
 <ul class="space-y-4 text-sm">
 <li><a href="#" class="hover:text-brand-400 transition-colors">{{ __('Documentation') }}</a></li>
