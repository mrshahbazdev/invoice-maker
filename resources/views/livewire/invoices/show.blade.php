@php $title = $invoice->isEstimate() ? 'View Estimate' : 'View Invoice'; @endphp

<div>
    <div class="mb-8 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
            <h2 class="text-2xl font-bold text-gray-900">{{ $invoice->isEstimate() ? 'Estimate' : 'Invoice' }} Details</h2>
        ...
        <div class="flex flex-wrap gap-2">
            ...
            <button wire:click="sendEmail"
                ...
                Email {{ $invoice->isEstimate() ? 'Estimate' : 'Invoice' }}
            </button>
            ...
            @if($invoice->status === 'draft')
                <a href="{{ $invoice->isEstimate() ? route('estimates.edit', $invoice) : route('invoices.edit', $invoice) }}"
                    ...
                    Edit {{ $invoice->isEstimate() ? 'Estimate' : 'Invoice' }}
                </a>
            @endif
        ...
        <div class="lg:col-span-2 space-y-6">
            <div class="bg-white rounded-lg shadow p-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    ...
                    <div>
                        <h4 class="text-xs font-bold text-gray-400 uppercase tracking-wider mb-3">{{ $invoice->isEstimate() ? 'ESTIMATE' : 'INVOICE' }} TO</h4>
                        <div>
                ...
                <h4 class="text-xs font-bold text-gray-400 uppercase tracking-wider mb-3">{{ $invoice->isEstimate() ? 'ESTIMATE' : 'INVOICE' }} INFO</h4>
                <div class="space-y-1">
                    <div class="flex justify-between text-sm">
                        <span class="text-gray-500">{{ $invoice->isEstimate() ? 'Estimate' : 'Invoice' }} #:</span>
                        <span class="font-medium text-gray-900">{{ $invoice->invoice_number }}</span>
                    </div>
                    <div class="flex justify-between text-sm">
                        <span class="text-gray-500">{{ $invoice->isEstimate() ? 'Estimate' : 'Invoice' }} Date:</span>
                        <span class="font-medium text-gray-900">{{ $invoice->invoice_date->format('M d, Y') }}</span>
                    </div>
                    <div class="flex justify-between text-sm">
                        <span class="text-gray-500">{{ $invoice->isEstimate() ? 'Expiration' : 'Due' }} Date:</span>
                            ...
                            @foreach($invoice->items as $item)
                                <tr class="border-b">
                                    <td class="py-3">
                                        ...
                                        @if($item->tax_rate > 0)
                                        <p class="text-sm text-gray-500">Tax: {{ $item->tax_rate }}%</p>@endif
                                    </td>
                        ...
                        @foreach($invoice->payments as $payment)
                            <div class="flex justify-between items-start pb-3 border-b last:border-0">
                                ...
                                <div class="text-right">
                                    <p class="text-sm text-gray-600">{{ ucfirst(str_replace('_', ' ', $payment->method)) }}</p>
                                    <button wire:click="deletePayment({{ $payment->id }})" wire:confirm="Delete this payment?"
