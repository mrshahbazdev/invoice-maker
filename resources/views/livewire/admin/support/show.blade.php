<div>
 ...
 <span>Opened by <span class="text-gray-300 font-medium">{{ $ticket->user->name }}</span> ({{ $ticket->user->email }})</span>
 <span>&bull;</span>
 ...
 <div class="bg-green-500/10 border border-green-500/20 text-green-400 px-4 py-3 rounded-xl flex items-center">
 ...
 @else
 <div class="h-10 w-10 rounded-full bg-gray-700 flex items-center justify-center text-white font-bold text-lg">
 {{ substr($reply->user->name, 0, 1) }}
 </div>
 @endif
 <div>
 ...
 <div class="prose prose-invert max-w-none text-gray-300 text-sm whitespace-pre-wrap">{{ $reply->message }}</div>
 
 @if($reply->attachment_path)
 <div class="mt-4 pt-4 border-t border-gray-700/50">
 <h4 class="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">Attachment</h4>
 <a href="{{ Storage::url($reply->attachment_path) }}" target="_blank" class="inline-flex items-center px-4 py-2 bg-gray-900 border border-gray-700 shadow-sm text-sm font-medium rounded-lg text-gray-300 hover:bg-gray-700 hover:text-white transition-colors">
 ...
 <label class="cursor-pointer bg-gray-900 py-2 px-3 border border-gray-700 rounded-lg shadow-sm text-sm leading-4 font-medium text-gray-300 hover:bg-gray-700 transition-colors">
 <span class="flex items-center">
 <svg class="h-4 w-4 mr-1.5 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.172 7l-6.586 6.586a2 2 0 102.828 2.828l6.414-6.586a4 4 0 00-5.656-5.656l-6.415 6.585a6 6 0 108.486 8.486L20.5 13" /></svg>
 Attach File
 </span>
 ...
 <div class="bg-gray-800 rounded-2xl border border-gray-700 p-6 shadow-sm sticky top-24">
 ...
 <div class="pt-4 border-t border-gray-700 font-mono text-xs text-gray-500 mb-4 px-1 space-y-2">
 <div><span class="text-gray-400">Last Replied:</span> {{ $ticket->updated_at->diffForHumans() }}</div>
 <div><span class="text-gray-400">Total Replies:</span> {{ $replies->count() }}</div>
 </div>

 <button type="submit" class="w-full inline-flex justify-center items-center rounded-xl bg-gray-700 py-2.5 px-4 text-sm font-bold text-white hover:bg-gray-600 transition-colors shadow-sm">
