...
class Languages extends Component
{
    ...
    public function mount()
    {
        ...
        $this->customLocales = \App\Models\Setting::get('site.custom_locales', []);
    ...
    public function toggleLanguage($code, $name)
    {
        ...
        session()->flash('message', 'Language status updated.');
    }
    ...
    {
        $this->validate([
            'newLanguageCode' => 'required|string|min:2|max:5',
        ...
        if (isset($this->availableLocales[$code])) {
            session()->flash('error', 'Language code already exists.');
            return;
        }

        $this->customLocales[$code] = $name;
        \App\Models\Setting::set('site.custom_locales', $this->customLocales, 'json');
    ...
    public function autoTranslate()
    {
        ...
        $targetName = $this->availableLocales[$this->editingLocale] ?? $this->editingLocale;

        // Skip keys that already have a non-empty translation
        $keysToTranslate = [];
        $stringsToTranslate = [];
        foreach ($this->baseTranslations as $key => $val) {
            if (empty(trim($this->translations[$key] ?? ''))) {
                $keysToTranslate[] = $key;
                $stringsToTranslate[$key] = $val;
            }
        }

        if (empty($stringsToTranslate)) {
            session()->flash('message', 'All phrases are already translated.');
            return;
        }
        ...
        try {
            ...
            $translated = $aiService->translateJson($stringsToTranslate, $targetName);

            foreach ($translated as $key => $val) {
                if (isset($this->baseTranslations[$key])) {
            ...
            session()->flash('message', 'Missing translations were generated by AI successfully.');
        } catch (\Exception $e) {
            Log::error($e->getMessage());
            session()->flash('error', 'AI Translation failed. Please check your AI API keys in settings.');
        }
    ...
    public function saveTranslations($showFlash = true)
    {
        if (!$this->editingLocale)
            return;
